...
func getCofactor(A, temp [][]float64, p, q, n int) {
	i := 0
	j := 0
...
func multyplyMatrices(m1 [][]float64, m2 [][]float64) ([][]float64, error) {
	if len(m1[0]) != len(m2) {
...
func main() {
	...
	col := row
	if col <= 0 {
		fmt.Println("Need position matrix dimensions!")
		return
	}
	...
	r1, err := multyplyMatrices(m1, inverse)
	if err != nil {
		fmt.Println(err)
		return
	}
